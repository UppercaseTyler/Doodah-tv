                ...
                while self.running and self.retry_count < self.max_retries and not url_failed and not self.needs_stream_switch:
                    ...
                    try:
                        ...
                        if connection_result:
                            ...
                            # A health/buffering monitor may request a reconnect while the
                            # current connection is still technically open. Consume that
                            # request here, then let the normal retry loop reopen self.url.
                            if getattr(self, 'needs_reconnect', False):
                                logger.info(f"Reconnect requested for current stream on channel {self.channel_id}")
                                self.needs_reconnect = False
                    ...
                    if self.buffering_start_time is None:
                        ...
                        if buffering_duration > self.buffering_timeout:
                            ...
                            if self._try_next_stream():
                                ...
                                logger.info(f"Requesting reconnect to current stream for channel {self.channel_id}")
    ...
    def _process_stream_data(self):
        ...
        try:
            ...
            while (
                ...
                and not getattr(self, 'needs_reconnect', False)
            ):
                if self.fetch_chunk():
                    self.last_data_time = time.time()
                else:
